      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            {plan.part_name && (
              <p className="text-muted-foreground mt-1">Część: {plan.part_name}</p>
            )}
            {plan.description && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Measurement Form */}
          <div>
            <h2 className="text-xl font-semibold text-foreground mb-4">Wprowadź pomiary</h2>
            <MeasurementForm
              planId={plan.id}
              items={sortedItems}
              orders={orders || []}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              <h3 className="text-lg font-semibold text-foreground mb-3">Wymiary w planie ({sortedItems.length})</h3>
              <div className="space-y-2">
                {sortedItems.map((item: {
                  ...
                  <div
                    key={item.id}
                    className={`p-3 rounded-lg ${item.is_critical ? 'bg-red-900/20 border border-red-700/50' : 'bg-muted'}`}
                  >
            ...
            {/* Recent Measurements */}
            <div className="bg-card border border-border rounded-lg p-4">
              <h3 className="text-lg font-semibold text-foreground mb-3">Ostatnie pomiary</h3>
                ...
                <div className="max-h-[400px] overflow-y-auto">
                  <table className="w-full">
                    <thead className="bg-muted sticky top-0">
                      <tr>
                        <th className="px-3 py-2 text-left text-xs text-foreground">Wymiar</th>
                        <th className="px-3 py-2 text-left text-xs text-foreground">Pomiar</th>
                        <th className="px-3 py-2 text-left text-xs text-foreground">Wynik</th>
                      </tr>
                    </thead>
