      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          <div className="bg-card rounded-lg border border-border p-4">
            <p className="text-muted-foreground text-xs mb-1">Wszystkie</p>
            <p className="text-2xl font-bold text-foreground">{summary.total}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4">
            <p className="text-muted-foreground text-xs mb-1">Oczekujące</p>
            <p className="text-2xl font-bold text-yellow-400">{summary.pending}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4">
            <p className="text-muted-foreground text-xs mb-1">W realizacji</p>
            <p className="text-2xl font-bold text-violet-400">{summary.in_progress}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4">
            <p className="text-muted-foreground text-xs mb-1">Ukończone</p>
            <p className="text-2xl font-bold text-green-400">{summary.completed}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4">
            <p className="text-muted-foreground text-xs mb-1">Opóźnione</p>
            <p className="text-2xl font-bold text-red-400">{summary.delayed}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4">
            <p className="text-muted-foreground text-xs mb-1">Przychód</p>
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
                  ...
                  <td className="px-4 py-3 text-foreground">{order.part_name || '-'}</td>
        ...
        {filteredOrders.length === 0 && (
          <div className="p-12 text-center">
            <p className="text-muted-foreground">Brak zamówień spełniających kryteria filtrowania</p>
          </div>
        )}
        ...
        <div className="px-4 py-3 bg-slate-100 dark:bg-background border-t border-border">
          <p className="text-muted-foreground text-sm">
            Wyświetlono <span className="text-foreground font-semibold">{filteredOrders.length}</span> zamówień
          </p>
        </div>
      </div>
    </div>
