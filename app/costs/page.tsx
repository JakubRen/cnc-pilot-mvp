      ...
      <div className="p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-4xl font-bold text-white">Analiza Kosztów i Rentowności</h1>
              <p className="text-slate-400 mt-1">Ostatnie {days} dni • {totalOrders} zamówień</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-blue-400">{totalRevenue.toFixed(0)} PLN</p>
            </div>

            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Koszt całkowity</p>
              <p className="text-2xl font-bold text-white">{totalCost.toFixed(0)} PLN</p>
            </div>

            <div className={`rounded-lg p-4 ${totalProfit >= 0 ? 'bg-green-900/30 border border-green-700' : 'bg-red-900/30 border border-red-700'}`}>
              <p className="text-slate-400 text-sm">Zysk</p>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Rentowne</p>
              <p className="text-2xl font-bold text-green-400">{profitableOrders}</p>
              <p className="text-slate-500 text-xs">{unprofitableOrders} nierentownych</p>
            </div>

            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Godziny pracy</p>
              <p className="text-2xl font-bold text-white">{totalLaborHours.toFixed(1)}h</p>
              <p className="text-slate-500 text-xs">{avgLaborCostPerHour.toFixed(0)} PLN/h śr.</p>
            </div>

            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Bez ceny</p>
              <p className="text-2xl font-bold text-yellow-400">{noSellingPrice}</p>
              <p className="text-slate-500 text-xs">brak analizy</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-slate-400">🔩 Materiały</span>
                <span className="text-white font-bold">{totalMaterialCost.toFixed(0)} PLN</span>
              </div>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-slate-400">⏱️ Praca</span>
                <span className="text-white font-bold">{totalLaborCost.toFixed(0)} PLN</span>
              </div>
              <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                <div
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-slate-400">🏭 Ogólne</span>
                <span className="text-white font-bold">{totalOverheadCost.toFixed(0)} PLN</span>
              </div>
              <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                <div
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-700">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-300 uppercase">Zamówienie</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-300 uppercase">Klient</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-300 uppercase">Status</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-slate-300 uppercase">Koszt</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-slate-300 uppercase">Cena</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-slate-300 uppercase">Marża</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-slate-300 uppercase">Godziny</th>
                  </tr>
                </thead>
                      ...
                      return (
                        <tr key={order.id} className="hover:bg-slate-700/50">
                          <td className="px-4 py-3">
                            <Link href={`/orders/${order.id}`} className="hover:text-blue-400 transition">
                              <span className="text-white font-medium">{order.order_number}</span>
                              {order.part_name && (
                                <span className="text-slate-500 text-sm block">{order.part_name}</span>
                              )}
                            </Link>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right">
                            <span className="text-white font-medium">{(order.total_cost || 0).toFixed(2)}</span>
                          ...
                          <td className="px-4 py-3 text-right">
                            {hasPrice ? (
                              <span className="text-blue-400 font-medium">{order.selling_price?.toFixed(2)}</span>
                            ) : (
                          ...
                          <td className="px-4 py-3 text-right">
                            {hasPrice ? (
                              <div>
                                ...
                                <span className={`text-xs block ${isProfitable ? 'text-green-500' : 'text-red-500'}`}>
                                  {margin.toFixed(1)}%
                                </span>
                              </div>
                            ) : (
