      ...
      <div className="p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-4xl font-bold text-white">Kontrola Jakości</h1>
              <p className="text-slate-400 mt-1">Plany kontroli, pomiary, raporty</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Plany kontroli</p>
              <p className="text-3xl font-bold text-white">{plans?.length || 0}</p>
            </div>
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Pomiary (ostatnie 20)</p>
              <p className="text-3xl font-bold text-white">{totalMeasurements}</p>
            </div>
            <div className="bg-slate-800 border border-green-700/50 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Zgodne</p>
            ...
            <div className="bg-slate-800 border border-red-700/50 rounded-lg p-4">
              <p className="text-slate-400 text-sm">Niezgodne</p>
          ...
          {totalMeasurements > 0 && (
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 mb-8">
              <div className="flex justify-between items-center mb-2">
                <span className="text-slate-300 font-medium">Wskaźnik zgodności</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* QC Plans */}
            <div>
              <h2 className="text-xl font-semibold text-white mb-4">Plany Kontroli</h2>
              {!plans || plans.length === 0 ? (
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 text-center">
                  <div className="text-5xl mb-4">📋</div>
                  <p className="text-slate-400 mb-4">Brak planów kontroli</p>
                  <Link href="/quality-control/plans/add">
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {plan.part_name && (
                            <p className="text-slate-400 text-sm">Część: {plan.part_name}</p>
                          )}
                        </div>
            ...
            {/* Recent Measurements */}
            <div>
              <h2 className="text-xl font-semibold text-white mb-4">Ostatnie Pomiary</h2>
              {!recentMeasurements || recentMeasurements.length === 0 ? (
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 text-center">
                  <div className="text-5xl mb-4">📏</div>
                  <p className="text-slate-400">Brak pomiarów</p>
                </div>
              ) : (
                        ...
                        return (
                          <tr key={measurement.id} className="hover:bg-slate-700/50">
                            <td className="px-4 py-2">
                              <p className="text-white text-sm">{item?.name || 'Nieznany'}</p>
