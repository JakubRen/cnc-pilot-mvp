        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              {plan.part_name && (
                <p className="text-slate-500 dark:text-slate-400 mt-1">Część: {plan.part_name}</p>
              )}
              {plan.description && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Measurement Form */}
            <div>
              <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Wprowadź pomiary</h2>
              <MeasurementForm
                planId={plan.id}
                items={sortedItems}
                orders={orders || []}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-3">Wymiary w planie ({sortedItems.length})</h3>
                <div className="space-y-2">
                  {sortedItems.map((item: {
                    ...
                    <div
                      key={item.id}
                      className={`p-3 rounded-lg ${item.is_critical ? 'bg-red-900/20 border border-red-700/50' : 'bg-slate-50 dark:bg-slate-900'}`}
                    >
              ...
              {/* Recent Measurements */}
              <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-3">Ostatnie pomiary</h3>
                  ...
                  <div className="max-h-[400px] overflow-y-auto">
                    <table className="w-full">
                      <thead className="bg-slate-100 dark:bg-slate-700 sticky top-0">
                        <tr>
                          <th className="px-3 py-2 text-left text-xs text-slate-700 dark:text-slate-300">Wymiar</th>
                          <th className="px-3 py-2 text-left text-xs text-slate-700 dark:text-slate-300">Pomiar</th>
                          <th className="px-3 py-2 text-left text-xs text-slate-700 dark:text-slate-300">Wynik</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {recentMeasurements.map((m) => (
                          <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                            ...
                            <td className="px-3 py-2 text-sm text-slate-900 dark:text-white font-mono">
                              {m.measured_value}
                            </td>
