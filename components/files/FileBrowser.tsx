  ...
  return (
    <div>
      ...
      {viewMode === 'grid' && filteredFiles.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredFiles.map((file) => (
            <div
              key={file.id}
              className="bg-card border border-border rounded-lg overflow-hidden hover:border-slate-300 dark:hover:border-slate-600 transition group"
            >
              {/* Preview */}
              <div
                className="h-40 bg-slate-100 dark:bg-background flex items-center justify-center cursor-pointer relative overflow-hidden"
              ...
              {/* Info */}
              <div className="p-4">
                <h4 className="text-foreground font-semibold text-sm truncate mb-2" title={file.original_filename}>
                ...
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 px-3 py-1.5 bg-violet-600 hover:bg-violet-700 text-white text-xs rounded transition font-semibold"
                  >
                    Pobierz
                  </button>
                  <button
      ...
      {viewMode === 'list' && filteredFiles.length > 0 && (
        <div className="space-y-2">
          {filteredFiles.map((file) => (
            <div
              key={file.id}
              className="flex items-center gap-4 p-4 bg-card border border-border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition group"
            >
              <div className="text-3xl flex-shrink-0">{getFileIcon(file.mime_type)}</div>

              <div className="flex-1 min-w-0">
                <h4 className="text-foreground font-semibold text-sm truncate" title={file.original_filename}>
                  {file.original_filename}
                </h4>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={() => setPreviewFile(file)}
                  className="px-3 py-1.5 bg-muted hover:bg-accent text-foreground text-xs rounded transition"
                >
                  Podgląd
                </button>
                <button
                  onClick={() => handleDownload(file)}
                  className="px-3 py-1.5 bg-violet-600 hover:bg-violet-700 text-white text-xs rounded transition font-semibold"
                >
                  Pobierz
                </button>
                <button
      ...
      {previewFile && (
        ...
        >
          <div
            className="bg-card border border-border rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="text-foreground font-bold truncate">{previewFile.original_filename}</h3>
              <button
                onClick={() => setPreviewFile(null)}
            ...
            <div className="p-4 overflow-auto max-h-[calc(90vh-120px)]">
              {previewFile.mime_type?.startsWith('image/') ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">Podgląd niedostępny dla tego typu pliku</p>
                  <button
            ...
            <div className="flex items-center justify-between p-4 border-t border-border bg-muted">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => handleDownload(previewFile)}
                  className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg transition font-semibold"
                >
                  Pobierz
                </button>
                <button
