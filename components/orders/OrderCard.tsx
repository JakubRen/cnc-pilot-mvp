...
export function OrderCard({ order }: OrderCardProps) {
  ...
  return (
    <div className={`bg-card rounded-lg p-4 border border-border hover-lift cursor-pointer ${isOverdue ? 'border-l-4 border-l-red-500' : ''}`}>
      {/* Header */}
      <div className="flex justify-between items-start mb-3">
        <div>
          <Link
            href={`/orders/${order.id}`}
            className="font-bold text-foreground hover:text-violet-600 dark:hover:text-violet-400 text-lg"
          >
            {order.order_number}
          </Link>
      ...
      <div className="space-y-2 text-sm">
        {order.part_name && (
          <div className="flex justify-between">
            ...
            <span className="text-foreground font-medium">{order.part_name}</span>
          </div>
        )}
        <div className="flex justify-between">
          <span className="text-muted-foreground">Ilość:</span>
          <span className="text-foreground font-medium">{order.quantity} szt.</span>
        </div>
        {order.deadline && (
          <div className="flex justify-between">
            <span className="text-muted-foreground">Termin:</span>
            <span className={isOverdue ? 'text-red-600 font-bold' : 'text-foreground font-medium'}>
